<%@ Page Title="Create Module - Admin"
    Language="C#"
    MasterPageFile="~/MasterPages/dashboard.Master"
    AutoEventWireup="true"
    CodeBehind="admin_create_module.aspx.cs"
    Inherits="RookiesInTraining2.Pages.admin_create_module" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="DashboardContent" runat="server">
    ...
    <style>
        ...
        .wizard-card { border: 1px solid #e8ecf1; box-shadow: 0 2px 8px rgba(0,0,0,0.06); border-radius: 1rem; overflow: hidden; background: white; }
        .wizard-header { background: white; border-bottom: 2px solid #e8ecf1; color: #2d3748; padding: 2rem; }
        .wizard-header h2 { color: #2d3748 !important; }
        .wizard-header p { color: #6c757d; }
        .wizard-nav { display: flex; justify-content: space-between; margin-top: 1.5rem; position: relative; }
        .wizard-nav::before { content: ''; position: absolute; top: 20px; left: 10%; right: 10%; height: 2px; background: #e8ecf1; }
        .wizard-step { position: relative; z-index: 1; display: flex; flex-direction: column; align-items: center; gap: 0.5rem; }
        .wizard-step .step-num { width: 40px; height: 40px; border-radius: 50%; background: #f8f9fa; border: 2px solid #e8ecf1; color: #6c757d; display: flex; align-items: center; justify-content: center; font-weight: 700; transition: all 0.3s; }
        .wizard-step.active .step-num { background: #667eea; border-color: #667eea; color: white; box-shadow: 0 0 0 4px rgba(102,126,234,0.1); transform: scale(1.1); }
        .wizard-step .step-label { font-size: 0.875rem; color: #6c757d; font-weight: 600; }
        .wizard-step.active .step-label { color: #667eea; }
        .wizard-body { padding: 2.5rem; min-height: 500px; background: white; }
        .step-content { display: none; }
        .step-content.active { display: block; animation: fadeIn 0.3s; }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .level-card { background: #f8f9fa; border: 2px solid #e8ecf1; border-radius: 0.75rem; padding: 1.5rem; margin-bottom: 1rem; position: relative; }
        ...
        .icon-picker label { width: 50px; height: 50px; border: 2px solid #e8ecf1; border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 1.5rem; transition: all 0.2s; background: white; color: #6c757d; }
        .icon-picker label:hover { border-color: #667eea; background: #f0f4ff; }
        .icon-picker input:checked + label { border-color: #667eea; background: #667eea; color: white; transform: scale(1.1); }
        .color-picker label { width: 45px; height: 45px; border-radius: 50%; cursor: pointer; border: 3px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.1); transition: all 0.2s; }
        .color-picker input:checked + label { box-shadow: 0 0 0 3px #667eea; transform: scale(1.15); }
        .review-box { background: #f8f9fa; border: 1px solid #e8ecf1; border-radius: 0.75rem; padding: 1.5rem; margin-bottom: 1.5rem; }
        .review-item { padding: 0.75rem 0; border-bottom: 1px solid #e8ecf1; display: flex; justify-content: space-between; }
        .review-item:last-child { border-bottom: none; }
        .review-item span:first-child { color: #6c757d; }
        .review-item strong { color: #2d3748; }
        .level-list-item { background: white; padding: 1rem; border-radius: 0.5rem; margin-bottom: 0.75rem; display: flex; align-items: center; gap: 1rem; border: 1px solid #e8ecf1; }
        ...
        .card-footer { background: white; border-top: 1px solid #e8ecf1; }
        h2, h3, h4, h5, h6 { color: #2d3748 !important; }
            ...
            <div class="card-footer bg-white border-top p-4">
                <div class="d-flex justify-content-between">
                    ...
                    <div class="ms-auto d-flex gap-2">
                        <a href="<%= ResolveUrl("~/Pages/admin/Classes.aspx") %>" class="btn btn-lg btn-outline-secondary">
